...
export default function SubjectPerformanceChart({ data }: Props) {
  const chartData = data.map(item => ({
    ...item,
    color: SUBJECT_COLORS[item.subject as keyof typeof SUBJECT_COLORS] || '#6b7280'
  }))
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-600">
            Average Score: <span className="font-medium text-primary-600">{data.score}%</span>
          </p>
          <p className="text-sm text-gray-600">
            Tests Taken: <span className="font-medium">{data.tests}</span>
          </p>
        </div>
      )
    }
